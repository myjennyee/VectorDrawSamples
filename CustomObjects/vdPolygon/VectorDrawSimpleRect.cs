...
//This custom object also demonstrates how you can serialize your own custom class in vdml/vdcl

namespace VectorDrawCustomFigs
{
    public class vdSuperPoly : vdPolyline, IvdProxyFigure
    {
        public vdSuperPoly()
        {
        }
        public vdSuperPoly(vdDocument Doc)
        {
        ...
        {
            if (ListStart(render)) return render.StatusDraw;// For version 7000
            vdRender.DrawStatus doDraw = base.Draw(render);
            if ((doDraw == vdRender.DrawStatus.Successed) && (VertexList.Count > 2))
            {
                render.PushDrawFigureList(this);
                bool needpush = !ECSMatrix.IsUnitMatrix();
                if (needpush) render.PushMatrix(ECSMatrix);
                render.PushPenstyle(GetRenderPenStyle(render));
                gPoint pt = this.VertexList[0];
                gPoint pt2 = this.VertexList[2];
                render.DrawLine(this, pt, pt2);     //draw the diagonal
                render.PopPenstyle();
                if (needpush) render.PopMatrix();
                render.PopDrawFigureList();

            }
            if (ListEnd(render)) doDraw = render.StatusDraw;// For version 7000
            return doDraw;
        }
    }
    ...
    public class VectorDrawSimpleRect : vdCurve, IvdProxyFigure, IvdProxySerializer, IVDSerialise
    {
        #region variables
        ...
        public VectorDrawSimpleRect(vdDocument doc)
            : base()
        {
            SetUnRegisterDocument(doc);
            setDocumentDefaults();
...
//-----------------------------------------------------------------------------------------------------------------------------
        ...
        {
            vdRect rect = new vdRect(this.Document, this.InsertionPoint, this.Width, this.Height, this.Rotation);
            rect.SetUnRegisterDocument(this.Document);
            rect.MatchProperties(this, this.Document);
            this.Deleted = true;
            
            vdEntities collection = Owner as vdEntities;
            collection.AddItem(rect);
            return rect.Break(p1, p2, out newEntity);
        }
        ...
        public override vdEntities Explode()
        {
            ...
            pl.HatchProperties = new vdHatchProperties(VectorDraw.Professional.Constants.VdConstFill.VdFillModeSolid);
            pl.HatchProperties.Solid2dTransparency = 50;
            pl.HatchProperties.FillColor.ColorIndex = 2;
            Entities.AddItem(pl);
        ...
        public override bool IntersectWith(VectorDraw.Professional.vdPrimaries.vdFigure pEntity, VectorDraw.Professional.Constants.VdConstInters intersectOption, gPoints intPoints)
        {
            ...
            vdEntities mEnts = new vdEntities();
            mEnts = this.Explode();
            if (mEnts == null) return false;
            foreach (vdFigure var in mEnts)
            {
                ret = (var.IntersectWith(pEntity, intersectOption, intPoints) || ret);
            }
            return ret;
        }
